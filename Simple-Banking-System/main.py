...
class Account:
    ...
    def unique_ending(cls):
        ending = str(random.randint(0, 1000000000))
    ...
    @staticmethod
    def luhn_number(card_number):
        digits = [2 * int(x) if card_number.index(x) % 2 != 0 else int(x) for x in card_number]
        digits = [x - 9 if x > 9 else x for x in digits]
        return str((10 - (sum(digits) % 10)) % 10)

    def __init__(self, cls):
        ending = cls.unique_ending(cls)
        if len(ending) < 9:
            ending = "0" * (9 - len(ending)) + ending
        self.number = "400000" + ending + cls.luhn_number("400000" + ending)
